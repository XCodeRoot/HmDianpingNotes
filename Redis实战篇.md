...
#### ==yaml配置==(包括mysql,redis,port,设置日志级别等)
...
```java
   @Override
    public Result sentCode(String phone, HttpSession session) {
        ...
        //3.符合就生成验证码 (利用万能工具库 hutool)
        String code = RandomUtil.randomNumbers(6);
        //4.保存session
        session.setAttribute("code",code);
        //5.发送验证码(模拟发送,输出到日志,因为不可能真的去发送给手机验证码)
...
```java
    ...
    public Result login(LoginFormDTO loginForm, HttpSession session) {
        ...
        if(RegexUtils.isPhoneInvalid(phone)){//从请求体 里取出phone字段
            //2.如果不符合,就报错
            return Result.fail("手机号格式错误!");
        }
        //3.校验验证码
        Object cacheCode = session.getAttribute("code");//实际的
        String code= loginForm.getCode();//表单的
        ...
        //7.无论是有没有查到,都把这个 老用户/新用户 保存到session
        //利用工具类把User的属性复制到UserDTO里
        //让session保存尽可能少的信息 , 避免不必要的内存,也避免信息泄露
...
#### 1.7.1、==设计key的结构----使用hash结构: key-field-value==
...
### 1.8 ==基于Redis实现短信登录 (redis做两个事 :一是临时保存验证码,二是临时保存登录信息)==

​	一 . redis里 以string结构,保存 以 `login:code: `为前缀的key  , 再 加 手机号 作为整个key ; value为验证码 

​	二 . redis 里 以hash结构 key field value , key的前缀为 `login:token:`  外加随机字符串作为整个key , 多个field和value , 还原成对象就是 UserDTO
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result sentCode(String phone, HttpSession session) {
        ...
        //3.符合就生成随机数验证码 (利用万能工具库 hutool)
        String code = RandomUtil.randomNumbers(6);
        //4.保存验证码到 Redis  :  set key value  保存2分钟
    ...
    public Result login(LoginFormDTO loginForm, HttpSession session) {
        ...
        //3.从Redis获取验证码并校验 , 根据前缀加手机号来从redis里取出 验证码
        ...
        //7.保存用户信息到redis里
            ...
            String token = UUID.randomUUID().toString(true);//hutool的工具类
        ...
        //8.返回token
        return Result.ok(token);//把token传给了前端,我们从前端的请求头中拿到 authorization ,这个就是token
    }
...
public class RefreshTokenInterceptor implements HandlerInterceptor{
    ...
    public RefreshTokenInterceptor(StringRedisTemplate stringRedisTemplate) {//普通类 , 不能使用容器里的bean,需要别人传给你才行
        this.stringRedisTemplate = stringRedisTemplate;
    }

    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        //1.获取请求头中的token , 我们在LoginServiceImpl里,将token回传给了前端,前端将token保存在了请求头的authorization里
        String token = request.getHeader("authorization");//我们通过这个authorization获取到登录令牌token,加上前缀就可以校验登录信息
        ...
        //6.存在,就保存到ThreadLocal , 封装方法在dto包里
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
        UserHolder.removeUser();//移除保存在线程池里的用户
    }
}
```
...
public class LoginInterceptor implements HandlerInterceptor{
    ...
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        //1.判断是否要拦截( 根据ThreadLocal中是否有这个用户)
...
public class MvcConfig implements WebMvcConfigurer {
    ...
    @Resource
    private StringRedisTemplate stringRedisTemplate;//自动装配,给拦截器用

    @Override
    public void addInterceptors(InterceptorRegistry registry) {
        //2.登录拦截器 , 第二个拦截器, 只放行登录成功的用户
        registry.addInterceptor(new LoginInterceptor())//new构造器来注册拦截器
                .excludePathPatterns( // 放行下面所有请求
                        "/user/login",
                        "/user/code",
                        "/shop/**",
                        "/shop-type/**",
                        ...
                        "/upload/**",
                        "/voucher/**"
                ).order(1);//第二个拦截器 , 进行拦截功能,做登录校验

        //1.token刷新的拦截器 , 先拦截所有,然后全放行,目的在于刷新token有效期,让正在使用的用户,一直保存登录状态不过期
        registry.addInterceptor(new RefreshTokenInterceptor(stringRedisTemplate))
                .addPathPatterns("/**").order(0);//第一个拦截器 , 拦截所有请求,只放行不拦截,并保存用户信息到ThreadLocal里做登录校验
    }
}
```
...
配置类 , 注册两个拦截器 , 先执行 `RefreshTokenInterceptor `后执行`LoginInterceptor`
...
public class ShopServiceImpl extends ServiceImpl<ShopMapper, Shop> implements IShopService {
    ...
    public Result queryById(Long id) {
        ...
        if(StrUtil.isNotBlank(shopJson)){
            //3.存在就直接返回给前端 , 先得把redis缓存里的JSON字符串转成 店铺对象
        ...
        //6.存在就写进redis缓存 , 先得把 店铺对象序列化成 json字符串
...
**==商品类型列表缓存==** ( 写不出来 , 初步构思 , redis的结构应该是使用hash结构  , key是 "shop_type" 自拟的 , field是 id 从1-10 , 每个 field里的id 对应一个value,就是 商品类型对象的序列化成的json字符串  , 关键在于 , 怎么从数据库里查出来一张表的所有的值, 把这个结果集放进redis里 
...
无论是先删缓存还是先操作数据库 , 都是有问题的 , 当两个线程 干扰时, 会发生读脏 , 只有两个线程 在不同的时间执行时,两个都正常

方案二出现问题的概率小 , 因为缓存突然失效的时候,用户查了一次缓存, 缓存未命中就去查数据库 , 紧接着线程二插进来, 更新数据库然后删除缓存, 线程一写入的缓存 , 是脏数据 . 查数据库是很慢的,写入缓存是很快的, 时间非常短, 此时插入一个线程进行更新操作的可能性比较小 , 所以方案二胜出
...
public Result queryById(Long id) {
    ...
    //6.存在就写进redis缓存 , 先得把 店铺对象序列化成 json字符串
    stringRedisTemplate.opsForValue().set(key,JSONUtil.toJsonStr(shop),CACHE_SHOP_TTL, TimeUnit.MINUTES);//30分钟缓存过期
    //7.最后返回店铺信息给前端
    return Result.ok(shop);
}
```

**修改重点代码2**  , ShopServiceImpl里的 update方法

代码分析：通过之前的淘汰，我们确定了采用删除策略，来解决双写问题，当我们修改了数据之后，然后把缓存中的数据进行删除，查询时发现缓存中没有数据，则会从mysql中加载最新的数据，从而避免数据库和缓存不一致的问题



==postman 发送 修改请求== , PUT , raw , json 
...
@Override
@Transactional
public Result update(Shop shop) {//因为我们没有后端界面 , 所以使用的是postman发送 update请求
    //先 更新数据库 , 后 删除缓存
    Long id=shop.getId();
    if(id==null){
...
@Override
    public Result queryById(Long id) {
        ...
        String shopJson = stringRedisTemplate.opsForValue().get(key);
        //2.判断是否存在于redis
        if(StrUtil.isNotBlank(shopJson)){//如果是""空串,则判断为redis不存在
            //3.存在就直接返回给前端 , 先得把redis缓存里的JSON字符串转成 店铺对象
            Shop shop = JSONUtil.toBean(shopJson, Shop.class);
            return Result.ok(shop);
        }
        //判断缓存命中的是否是空值 , ""空串!=null 不是空值 ,
        if(shopJson!=null){//如果不是空值 ,说明是空串"" , 代表是防止缓存穿透而生成的空串""
            //返回一个错误信息
        ...
        Shop shop = getById(id);
        if (shop==null){
            //5.数据库里不存在就报错 , 并 将空对象缓存进redis
            stringRedisTemplate.opsForValue().set(key,"",CACHE_NULL_TTL, TimeUnit.MINUTES);//30分钟缓存过期
            Result.fail("该店铺不存在!");
        }
        //6.数据库里存在就写进redis缓存 , 先得把 店铺对象序列化成 json字符串
        stringRedisTemplate.opsForValue().set(key,JSONUtil.toJsonStr(shop),CACHE_SHOP_TTL, TimeUnit.MINUTES);//30分钟缓存过期
        //7.最后返回店铺信息给前端
        return Result.ok(shop);
    }
...
核心思路就是利用==redis的setnx方法来表示获取锁==，该方法含义是redis中如果没有这个key，则插入成功，返回1，在stringRedisTemplate中返回true，  如果有这个key则插入失败，则返回0，在stringRedisTemplate返回false，我们可以通过true，或者是false，来表示是否有线程成功插入key，成功插入的key的线程我们认为他就是获得到锁的线程。

```java
// 互斥锁 , 用于应对 缓存击穿 造成原因:高并发key突然失效,缓存创建过程太长,导致所有请求全部打在数据库
private boolean tryLock(String key){
    Boolean flag = stringRedisTemplate.opsForValue().setIfAbsent(key, "1", LOCK_SHOP_TTL, TimeUnit.SECONDS);//10s 过期
    return BooleanUtil.isTrue(flag);//如果当前没有互斥锁则返回true
} 
// 解锁
...
当获取锁成功后 , 需要进行 doubleCheck , 二次确认 缓存是否还是没有

```java
//应对 缓存击穿, 互斥锁解决
//封装方法,供其他使用
//缓存击穿 造成原因:高并发key突然失效,缓存创建过程太长,导致所有请求全部打在数据库
public Shop queryWithMutex(Long id){
    ...
    try {
        ...
        //4.2.判断是否获取成功
        if (!isLock){
            //4.3.没获取到就休眠,然后重试 , 使用递归
            Thread.sleep(50);
            return queryWithMutex(id);
        }
        //4.4.二次确认 DoubleCheck , 如果此时缓存里有了该请求的数据,则直接返回缓存中的数据
        shopJson = stringRedisTemplate.opsForValue().get(key);
        if(StrUtil.isNotBlank(shopJson)){//如果是""空串,则判断为redis不存在
            //4.5.存在就直接返回给前端 , 先得把redis缓存里的JSON字符串转成 店铺对象
            return JSONUtil.toBean(shopJson, Shop.class);
        }
        //4.6.缓存里还是没有,就查数据库
        shop = getById(id);
...
1. 默认利用逻辑过期解决缓存击穿问题时 , 都是 搞大活动 ,  提前预热好了缓存, 而且默认缓存都是命中了的 , 只是分两种情况

   一是过期  : 

   ​					这个过期字段 属于 自定义的一个字段 , 并不是 TTL 那个过期时间 , 并不会真正的过期, 只是认为给一个过期字段, 
...
public class RedisData {
    ...
    private Object data;//传入任意数据类型 , 对原有代码没有侵入性 , 非常好
}
```
...
```
// 提前预热 redis缓存 , 用于大型活动的高并发 , 使用 逻辑过期解决方案
...
**ShopServiceImpl**   ,  获取锁成功后 , 要进行doubleCheck

```java

//创建线程池 , 用于开辟额外线程,进行缓存重建
...
public Shop queryWithLogicalExpire(Long id){
    ...
    String shopJson = stringRedisTemplate.opsForValue().get(key);
    //2.判断是否存在于redis
    if(StrUtil.isBlank(shopJson)){//如果是""空串,则判断为redis不存在
        //3.不存在就直接返回 null
        return null;
    }

    //4.缓存命中,就先把json反序列化成 对象
    RedisData redisData=JSONUtil.toBean(shopJson,RedisData.class);
    //因为RedisData里的data是 Object类型,需要转为JSONObject才能利用JSONUtil.toBean() 再此转为Shop类型
    ...
    //6.2.判断是否获取锁 成功
    if (isLock){
        //6.3.获取锁成功后 , 需要进行DoubleCheck , 再次取出缓存,判断缓存的逻辑时间是否过期
        redisData=JSONUtil.toBean(shopJson,RedisData.class);
        expireTime = redisData.getExpireTime();
        if (!expireTime.isAfter(LocalDateTime.now())){
            //如果还是过期的,就开启额外线程 , 进行缓存重建
 ...
 */
@Slf4j
@Component // 工具类 , 用于 set和get redis数据 , 解决 缓存穿透 和 缓存击穿问题
public class CacheClient {

    private  final StringRedisTemplate stringRedisTemplate;
    private static final ExecutorService CACHE_REBUILD_EXECUTOR= Executors.newFixedThreadPool(10);//线程池常量

    public CacheClient(StringRedisTemplate stringRedisTemplate) { //构造器  用于注入,容器里的stringRedisTemplate
        this.stringRedisTemplate = stringRedisTemplate;
    }
     ...
     * @param id 查询的id , 泛型 T 是id类型
     * @param type 返回类型 , 泛型 R
     * @param dbFallback Function<T,R> 是查询数据库getById(id)的函数 , T是id的类型,R是返回的结果集的类型
     * @param time TTL 过期时间
     * @param unit 过期时间的单位 ,秒或分
     * @param <R> 返回类型
     * @param <T> id 的类型
     * @return 返回一个 结果集,类型为 T
     */
    //应对 缓存击穿, 缓存空对象来解决
    //封装方法,供其他使用
    //缓存击穿 造成原因:请求的数据无缓存,直接打在数据库上
    public <R,T>R  queryWithPassThrough(String keyPrefix, T id, Class<R> type, Function<T,R> dbFallback
        ...
        //4.不存在就查数据库
        R r=dbFallback.apply(id);// 这个就是调用函数入参 , 执行 getById(id) 根据id查询数据库
        if (r==null){
            //5.数据库里不存在就报错 , 并 将空对象缓存进redis
            stringRedisTemplate.opsForValue().set(key,"",CACHE_NULL_TTL, TimeUnit.MINUTES);//30分钟缓存过期
            Result.fail("该店铺不存在!");
    ...
    //封装方法,供其他使用
    //缓存击穿 造成原因:高并发key突然失效,缓存创建过程太长,导致所有请求全部打在数据库
    public <R,T> R queryWithLogicalExpire(String keyPrefix, T id, Class<R> type, Function<T,R> dbFallback
            , Long time,TimeUnit unit){
        ...
        //2.判断是否存在于redis
        if(StrUtil.isBlank(json)){//如果是""空串,则判断为redis不存在
            //3.不存在就直接返回 null
            return null;
        }
    ...
    /** 方法5：根据指定的key查询缓存，并反序列化为指定类型，需要利用互斥锁缓存击穿问题
     *
     * @param keyPrefix
     * @param id
     * @param type
    ...
    //封装方法,供其他使用
    //缓存击穿 造成原因:高并发key突然失效,缓存创建过程太长,导致所有请求全部打在数据库
    public <R,T> R  queryWithMutex(String keyPrefix, T id, Class<R> type, Function<T,R> dbFallback
            , Long time,TimeUnit unit){
        ...
        String json = stringRedisTemplate.opsForValue().get(key);
        //2.判断是否存在于redis
        if(StrUtil.isNotBlank(json)){//如果是""空串,则判断为redis不存在
            //3.存在就直接返回给前端 , 先得把redis缓存里的JSON字符串转成 店铺对象
            return JSONUtil.toBean(json, type);
        }
        //判断缓存命中的是否是空值 , ""空串!=null 不是空值 ,
        if(json!=null){//如果不是空值 ,说明是空串"" , 代表是防止缓存穿透而生成的空串""
            //返回一个错误信息
            return null;
        }
...
public class ShopServiceImpl extends ServiceImpl<ShopMapper, Shop> implements IShopService {
    ...
    @Resource
    private CacheClient cacheClient;//装配CacheClient,才能使用它的方法


    @Override
    public Result queryById(Long id) {
        //缓存穿透 应对策略 ---- 请求的数据无缓存,直接打在数据库上,我们缓存空对象来应对 缓存穿透
...
​	  		**redis的自增 , 就是传入key, 将value 自增指定步长**



下面是生成的其中一个id , 观察一下就知道了 , 左边是十进制,右边是二进制 

```
89689070443167754 ==> 0001 0011 1110 1010 0011 1011 1100  0000 0000 0000 0000 0000 0000 0000 1010
```
...
public class RedisIdWorker {
    ...
    //开始时间戳  :  我们随意设置的时间 2023.1.1 0:0:0 距 1970年 有多少秒
    private static final  long BEGIN_TIMESTAMP= 1672531200L;

    private static final  long COUNT_BITS= 32;//左移32位

    private StringRedisTemplate stringRedisTemplate;//由调用者注入一个在容器里的bean : stringRedisTemplate
    ...
    /** 返回一个自增长的 全局id
     *      前置知识 : long类型一共 8字节 64 bit位       1 字节 8 bit
     *      我们需要的全局id ==> 二进制有 64位 , 使用long类型就行了 , 十进制 ==> 二进制
     *      1+31+32 == 64
     *      第一位是符号位, 固定为0
     *      高位有31位,放时间戳 , 这个十进制时间戳,转成二进制,未到31位0或1
     *      低位有32位,放序列号 , 也是未到32位0或1
     *
     * @param keyPrefix
     * @return  89689070443167754 ==> 0001 0011 1110 1010 0011 1011 1100  0000 0000 0000 0000 0000 0000 0000 1010
     */
    public long nextId(String keyPrefix){
        //1.生成时间戳 十进制
        LocalDateTime now = LocalDateTime.now();//返回了当前系统时区的日期时间
        long nowSecond = now.toEpochSecond(ZoneOffset.UTC);//当前时间 距 1970年 多少秒
        long timeStamp=nowSecond-BEGIN_TIMESTAMP;//最终时间戳,十进制
        //2.生成序列号 十进制
        //2.1.获取当前日期,精确年月日
        String date = now.format(DateTimeFormatter.ofPattern("yy:MM:dd"));// 一个年月日字符串
        //2.2.往redis设置 自增长键 increment
        //返回的这个count,就是redis从0开始以默认步长1,自增
        Long count = stringRedisTemplate.opsForValue()
                .increment("icr:" + keyPrefix + ":" + date);//以当前日期开始自增长,十进制

        //3.拼接起来并返回 , 使用位运算,将时间戳左移32位,这样就空出来32个0,用 或运算 , 就可以拼接起来序列号了
        return timeStamp<<COUNT_BITS | count;//位运算时,会将十进制转成二进制,运算完后,又返回十进制,所以返回值还是long类型
    }

    public static void main(String[] args) {
        //创建一个时间 , 可以随意 2023.1.1 0:0:0
        LocalDateTime time=LocalDateTime.of(2023,1,1,0,0,0);

        //Returns: the number of seconds from the epoch of 1970-01-01T00:00:00Z
        //算出这个时间距 1970-01-01T00:00:00Z 多少秒
...
==使用postman 添加秒杀券==   raw,json , POST

注意 : 设置的时间范围要在 现实的日期 , 否则添加了也不显示在网页里

{
  ...
  "beginTime":"2023-08-30T06:42:39",

  "endTime":"2023-08-31T19:43:31"
...
**使用 Jmeter 来测试 我们的库存超卖问题**

### ==Jmeter测试带有登录校验的请求地址,需要设置请求头authorization=redis里的token==

1. 当我们做了登录校验时, Jmeter请求将不能直接访问到 , 这时需要设置一下 Jmeter的 请求头 , 

   ​			让它包含 authorization = 我们redis里缓存的token信息

   ![](Redis实战篇.assets/1.png)

   ![](Redis实战篇.assets/2.png)
...
1. 要去添加依赖 
2. ==去主方法类, 暴露代理对象  `@EnableAspectJAutoProxy(exposeProxy = true)//暴露代理对象==
...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    @Resource
    private ISeckillVoucherService seckillVoucherService;//注册秒杀券的service

    @Resource
    private RedisIdWorker redisIdWorker;//注册这个工具类,获取方法,创建自增的全局id

    @Override
    public Result seckillVoucher(Long voucherId) {
        //1.查询秒杀券id
        SeckillVoucher voucher = seckillVoucherService.getById(voucherId);//仅仅只查一次
        //2.判断秒杀是否开始
        if (voucher.getBeginTime().isAfter(LocalDateTime.now())) {
            //秒杀还没开始,返回错误
        ...
        //5.创建订单  ( 先获取锁-->提交事务-->释放锁  ) ==> 确保线程安全
        //将用户id new成 字符串 , 放在常量池,只要id的值相同,就一定去常量池找同一个字符串
        Long userId = UserHolder.getUser().getId();
        //这样把创建订单的方法,加锁,就可以确保,解锁的时候一定执行了数据库的事务commit
        synchronized (userId.toString().intern()) {//id一样时,锁就一样,不同的用户就不会被锁定
            //获取当前类的代理对象,这样就不会让事务失效
    ...
    public  Result createVoucherOrder(Long voucherId) {
        ...
        voucherOrder.setId(orderId);
        //7.2.设置用户id
        voucherOrder.setUserId(userId);//上面一人一单的时候用过
        //7.3.设置代金券id
        voucherOrder.setVoucherId(voucherId);
        //7.4.写入数据库
        save(voucherOrder);
...
==**有关锁失效原因分析** :  一个jvm只有一个锁监视器 , 保证同时只有一个线程获取锁 , 实现线程直接的互斥 , 多个jvm的时候就有多个锁监视器了, 不同的jvm直接不能实现线程互斥, 咱们使用的是同步锁, 锁的是一个string,放进常量池==
...
* 获取锁：
  ...
  * 互斥：确保只能有一个线程获取锁
    ...
    `redis 的set lock thread1 NX EX10`
...
我们利用redis 的set lock thread1 NX EX10  方法，同时保证 获取锁和设置TTL时间的原子性, 防止redis服务宕机,未设置TTL时间
...
```lua
-- 这里的 KEYS[1] 就是锁的key，这里的ARGV[1] 就是当前线程标示,也就是锁的value
-- redis.call('get',KEYS[1]) 获取线程标识
...
```java
	//在编译时,就准备好导入 unlock.lua 这个脚本文件 , 这样就不用每次反复调用
    private static final DefaultRedisScript<Long> UNLOCK_SCRIPT;

    static{
        UNLOCK_SCRIPT= new DefaultRedisScript<>();//
        UNLOCK_SCRIPT.setLocation(new ClassPathResource("unlock.lua"));//导入 脚本文件
    ...
    public void unLock() {
        ...
        stringRedisTemplate.execute(
                UNLOCK_SCRIPT // 脚本
                , Collections.singletonList(KEY_PREFIX + name) // KEYS[1] , redis里锁的 key
...
* 创建一个Stream类型的消息队列，名为stream.orders
  ...
  ==必须先在redis里创建一个消息队列,       XGROUP CREATE stream.orders g1 0 MKSTREAM==
...
==注意：同学们在操作时，需要修改SystemConstants.IMAGE_UPLOAD_DIR 自己图片所在的地址，在实际开发中图片一般会放在nginx上或者是云存储上。public static final String IMAGE_UPLOAD_DIR = "D:\\SoftWare\\nginx-1.18.0\\html\\hmdp\\imgs\\";==
